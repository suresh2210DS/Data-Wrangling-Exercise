...
def filter_and_convert_to_csv(filename):
  ...
  if "review" in filename:
     ...
     for line in open(filename, 'r'):
        r = json.loads(line)
        outputWriter.writerow({'user_id': r['user_id'], 'business_id': r['business_id'], 'stars': r['stars'], 'date': r['date']})
     ...
     fields= ['business_id', 'city', 'name', 'state', 'categories', 'review_count', 'stars']     
     outputWriter = csv.DictWriter(outputFile, fieldnames = fields, lineterminator='\n')
     print "Converting " + filename + " to " + "yelp_academic_dataset_review.csv ..."
     ...
     for line in open(filename, 'r'):
        ...
        if "Restaurants" in categories:
           ...
           s = r['state']
           sl = s.encode('ascii', 'ignore')

           # Now write the result to a CSV file
           outputWriter.writerow({'business_id': r['business_id'], 'city': c1, 'name': n1, 'state': sl, 'categories': r['categories'], 
